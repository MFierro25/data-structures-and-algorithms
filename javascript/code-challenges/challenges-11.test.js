...
const count = (target, input) => {
  // Solution code here...
  return input.map( arr => arr.reduce( (prev, curr) => curr === target ? 1 + prev : prev, 0 ) ).reduce( (prev, curr) => curr + prev, 0 );
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.map( arr => arr.reduce( (prev, curr) => curr + prev, 0 ) ).reduce( (prev, curr) => curr + prev, 0 );
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce( (prev, curr) => Number(curr.height) < Number(prev.height) ? curr : prev ).name;
};
